...
def test_2d(args, net, test_loader):
    ...
    with open('bounding_box_predictions.csv', 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['Index', 'Prediction', 'Actual'])
        ...
        for idx, (obs_s, target_s, obs_p, target_p, target_c, label_c) in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                prediction = preds_p.tolist()
                actual = target_p.tolist()
                csv_writer.writerow([idx, prediction, actual])
